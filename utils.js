class Utils {
    ...
    static drawNeuralNetwork(nn){
        ...
        Utils.drawLabels(layer_1_x, layer_1_ys, -30, ['left sensor', 'middle-left sensor', 'middle sensor', 'middle-right sensor', 'right sensor', 'speed'], nn.inputs, false, RIGHT)
    ...
    static drawNodes(x, ys, neuron_size, values, highlightNeurons){        
        ...
        for(let i=0; i<ys.length; i++){
            ...
            if(values.length>0)
                text(values[i].toFixed(2).toString(), x, ys[i])
        }
    }
