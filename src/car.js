...
class Car {
    constructor(x, y, brain) {
        ...
        this.sensorsLength = Array(5).fill(this.sensorsRange);
    ...
    detectWalls(obstacles) {
        for (let i = 0; i < this.sensors.length; i++) {
            ...
            for (let j = 0; j < obstacles.length; j++) {
                let p2 = createVector(
                    this.frontPos.x + cos(this.sensors[i] + this.heading) * this.sensorsRange,
                    this.frontPos.y + sin(this.sensors[i] + this.heading) * this.sensorsRange
                );
                stroke(255);
    ...
    borders() {
        ...
        if (this.pos.y > height + this.h) this.pos.y = -this.h;
        if (this.pos.y < -this.h) this.pos.y = height + this.h;
    }
}
